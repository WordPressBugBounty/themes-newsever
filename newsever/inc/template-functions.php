...
function newsever_display_list_popular_taxonomies($popular_taxonomies = '', $title = "Popular Tags")
{
  ...
  if (isset($popular_taxonomies) && !empty($popular_taxonomies)):
    ...
    foreach ($popular_taxonomies as $tax_term):
      $html .= '<li>';
      $html .= '<a href="' . esc_url(get_term_link($tax_term)) . '" aria-lable="' . esc_attr($tax_term->name) . '">';
      $html .= $tax_term->name;
  ...
  if ($show_comment_count == 'yes'):
    ...
    if (absint($comment_count) > 1):
      ...
      <span class="min-read-post-comment">
        <a class="af-comment-count" href="<?php the_permalink(); ?>" aria-label="<?php echo esc_attr(get_the_id($post_id)); ?>">
...
if (!function_exists('newsever_athfb_add_custom_admin_menu')) {

  function newsever_athfb_add_custom_admin_menu($wp_admin_bar)
  {
    ...
    $parent_title  = $afthemes_icon . esc_html__('Newsever Options', 'newsever');

    // Add parent menu
    $wp_admin_bar->add_menu(array(
      'id'    => 'newsever-menu',
      'title' => $parent_title,
      'href'  => admin_url('admin.php?page=newsever-pro'),
      'meta'  => array(
        'title'  => esc_attr__('Newsever Options', 'newsever'), // Tooltip
    ...
    $submenu_items = array(
      ...
      array(
        ...
        'title' => __('Starter Sites', 'newsever'),
  ...
  add_action('admin_bar_menu', 'newsever_athfb_add_custom_admin_menu', 100);
  add_action('admin_enqueue_scripts', 'newsever_admin_bar_styling');
  add_action('wp_enqueue_scripts', 'newsever_admin_bar_styling'); // Also in frontend if admin bar visible

  function newsever_admin_bar_styling()
  {
    if (is_admin_bar_showing()) {
      wp_add_inline_style(
        ...
        #wpadminbar #wp-admin-bar-newsever-menu > .ab-item {
        ...
        #wpadminbar #wp-admin-bar-newsever-menu > .ab-item:hover,
        #wpadminbar #wp-admin-bar-newsever-menu.hover > .ab-item,
        #wpadminbar #wp-admin-bar-newsever-menu > .ab-item:focus,
        #wpadminbar #wp-admin-bar-newsever-menu > .ab-item:active {
            background-color: #006eb8 !important;
            color: #fff !important;
        }

        /* Visited state (rarely used in admin bar) */
        #wpadminbar #wp-admin-bar-newsever-menu > .ab-item:visited {
            color: #fff !important;
        }

        /* Icon alignment */
        // #wpadminbar #wp-admin-bar-newsever-menu img {
        //     vertical-align: middle;
        //     margin-right: 4px;
        //     color: #fff !important;
        // }
        #wpadminbar #wp-admin-bar-newsever-menu svg {
          height:20px;
          width: 20px;
          fill: #fff;
          color: #fff;
      ...
      #wpadminbar #wp-admin-bar-newsever-menu:hover svg {
          fill: #ffcc00;
      }
