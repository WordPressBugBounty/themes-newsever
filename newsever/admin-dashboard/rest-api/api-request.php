...
add_action('rest_api_init', 'newsever_register_plugins_routes');
function newsever_register_plugins_routes()
{
  register_rest_route('aft-useful-plugins/v1', '/get-useful-plugins', array(
    'methods' => 'GET',
    'callback' => 'newsever_get_all_useful_plugins',
...
function newsever_get_all_useful_plugins(\WP_REST_Request $request)
{
  ...
  foreach ($plugin_array as $plugin) {
    ...
    $button_text = __('Install Now', 'newsever');
    ...
    if (!is_wp_error($api)) { // confirm error free
      ...
      $main_plugin_file = newsever_get_plugin_file($plugin['slug']); // Get main plugin file
      ...
      if (newsever_check_file_extension($main_plugin_file)) { // check file extension
        if (is_plugin_active($main_plugin_file)) {
          // plugin activation, confirmed!
          $button_classes = 'button disabled';
          $button_text = __('Activated', 'newsever');
        } else {
          // It's installed, let's activate it
          $button_classes = 'activate button button-primary';
          $button_text = __('Activate', 'newsever');
        }
      }
      $data['plugins'][] = newsever_render_plugin_lists_template($plugin, $api, $button_text, $button_classes, $plugin_description, $title);
    }
  }
...
function newsever_render_plugin_lists_template($plugin, $api, $button_text, $button_classes, $plugin_description, $title)
{
  ...
  <div class="aft-plugin-installer">
    <div class="plugin">
      ...
      <div class="plugin-info">
        ...
        <p class="plugin-author"><?php _e('By', 'newsever'); ?> <?php echo $api->author; ?></p>
      </div>
      <ul class="activation-row">
        <li>
          <?php if ($api->slug == 'templatespare' && $button_text == 'Activated') { ?>
            <a class="button-primary" href="<?php echo site_url() . '/wp-admin/admin.php?page=newsever' ?>"><?php echo _e('Get Starter Sites', 'newsever') ?></a>
...
function newsever_get_plugin_file($plugin_slug)
{
...
function newsever_check_file_extension($filename)
{
