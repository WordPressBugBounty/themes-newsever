...
add_action('wp_ajax_newsever_plugin_installer', 'newsever_plugin_installer'); // Install plugin
add_action('wp_ajax_newsever_plugin_activation', 'newsever_plugin_activation'); // Activate plugin
add_action('wp_ajax_newsever_plugin_installer_activation', 'newsever_plugin_installer_activation'); // Activate plugin
function newsever_plugin_installer()
{

  if (! current_user_can('install_plugins'))
    wp_die(__('Sorry, you are not allowed to install plugins on this site.', 'newsever'));
  ...
  if (! wp_verify_nonce($nonce, 'aft_installer_nonce'))
    wp_die(__('Error - unable to verify nonce, please try again.', 'newsever'));
...
function newsever_plugin_activation()
{
  if (! current_user_can('install_plugins'))
    wp_die(__('Sorry, you are not allowed to activate plugins on this site.', 'newsever'));
  ...
  // Check our nonce, if they don't match then bounce!
  if (! wp_verify_nonce($nonce, 'aft_installer_nonce'))
    die(__('Error - unable to verify nonce, please try again.', 'newsever'));
  ...
  if ($api->name) {
    $main_plugin_file = newsever_get_plugin_file($plugin);
    $status = 'success';
    if ($main_plugin_file) {
...
function newsever_plugin_installer_activation()
{
  ...
  if (!empty($list_of_plugins)) {
    $data =  newsever_activate_plugin_list($sanitinzed_activate);
  }
...
function newsever_activate_plugin_list($sanitinzed_activate)
{
  $count = 0;
