...
class AdminNotice
{
  ...
  function newsever_dashboard_notice_content()
  {
    ...
    // $is_elespare_pro = newsever_get_plugin_file($elepsare_pro);
    // $af_themes_info = new AF_themes_info();
    // $check_blockspare = $af_themes_info->newsever_check_blockspare_free_pro_activated();
    // $check_elespare = $af_themes_info->newsever_check_elespare_free_pro_activated();
    ...
    //   if (!empty($newsever_elementor_pro_installed)) {
    //     array_push($plugins, 'elementor-pro');
    //   }
    //   if (!empty($newsever_elementor_installed)) {
    ...
    if (empty($activate_plugins) && empty($install_plugin)) {
      $btn_class = '';
      $newsever_templatespare_url = site_url() . '/wp-admin/admin.php?page=' . $this->page_slug;
    ...
    $notice_header = sprintf(
      ...
      esc_html__('👋 Welcome, and Thank You!', 'newsever'),
      sprintf(
        esc_html__('%s is now active. We\'re here to help you turn your ideas into a beautiful, professional website — quickly and confidently.', 'newsever'),
        $this->theme_name
      )
    );
    ...
    $demo_link = "https://afthemes.com/products/newsever/#aft-view-starter-sites";
    ...
    $notice_starter_msg = sprintf(
      ...
      __('🚀 Start With One Click', 'newsever'),
      esc_html__('Choose from 100+ starter sites designed for blogs, magazines, businesses & more.
      Setup is simple — no coding required.', 'newsever'),
      $newsever_templatespare_url,
      $newsever_templatespare_title,
      esc_url($demo_link),
      esc_html__('View Demos', 'newsever'),
    ...
    $notice_external_msg = sprintf(
      '<div class="aft-notice-col-3">
			...
			<div class="aft-documentation-links">
				...
				<div>
					<a href="https://wordpress.org/support/theme/newsever/reviews/?filter=5" class="button" target="_blank">%6$s</a>
				</div>
			</div>
			</div>',
      __('🤝 We\'re Here for You', 'newsever'),
      esc_html__('Whether you\'re just starting or customizing your site.', 'newsever'),
      esc_html__('Documentation', 'newsever'),
      esc_html__('Videos', 'newsever'),
      esc_html__('Support', 'newsever'),
      esc_html__('Rate This Theme', 'newsever')
